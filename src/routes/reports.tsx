...
function ReportsPage() {
  ...
  const rows = attendanceToday.filter((r) => filter === "All" || r.status === filter);
  const present = attendanceToday.filter((r) => r.status === "Present").length;
  const late = attendanceToday.filter((r) => r.status === "Late").length;
  const absent = attendanceToday.filter((r) => r.status === "Absent").length;
  const totalHours = attendanceToday.reduce((s, r) => s + r.hours, 0);
  const totalKm = attendanceToday.reduce((s, r) => s + r.distanceKm, 0);
  const maxPresent = Math.max(...weeklyAnalytics.map((w) => w.present));
  ...
  const summary = [
    ...
    { label: "Avg hours", value: (totalHours / attendanceToday.length).toFixed(1), tone: "text-info" },
  ];
      ...
      <section className="px-4 pt-6">
        ...
        <div className="mt-3 rounded-2xl border border-border bg-card p-4">
          ...
          <p className="mt-3 border-t border-border pt-3 text-xs text-muted-foreground">
            Total distance covered today: <span className="font-semibold text-foreground">{totalKm} km</span>{" "}
            · Working hours: <span className="font-semibold text-foreground">{totalHours.toFixed(1)}h</span>
          </p>
        </div>
      </section>
          ...
          {teamPerformance.map((t) => (
            <li key={t.team} className="rounded-2xl border border-border bg-card p-4">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                {t.visits} visits · {t.distanceKm} km covered
              </p>
      ...
      <section className="px-4 pb-6 pt-6">
        ...
        <ul className="mt-3 space-y-2">
          {rows.map((r) => (
            ...
            >
              <Avatar initials={r.initials} src={r.photo} alt={r.name} size={40} />
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-semibold">{r.name}</span>
                <span className="block truncate text-xs text-muted-foreground">
                  {r.checkIn} – {r.checkOut} · {r.hours}h · {r.distanceKm} km
                </span>
              </span>
              <span
